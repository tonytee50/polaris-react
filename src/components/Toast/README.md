...
- If merchants delete an image, offer the option to [Undo] the deletion. If they miss it in the toast, they can still retrieve it from somewhere else.
...
- Be used with a duration of at least 10000 milliseconds for accessibility.
...
Using toasts for critical information that merchants must act on immediately is not recommended. Since toasts disappear on their own, cannot be easily accessed by the keyboard, and may appear out of proximity to the merchant’s current focus, they may be more difficult for merchants with low vision or low dexterity to access.

### Toasts with actions

Toast messages with an action should persist for at least 10000 milliseconds to give the merchant enough time to act on it.

Ensure that merchants can accomplish the action in the toast through another way, since the toast action may be difficult to reach for some users. If the toast action is not available elsewhere on the page, for example a retry action that reloads a section, that functionality should have a fallback action, for example a browser refresh.
